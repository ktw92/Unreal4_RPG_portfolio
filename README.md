...
6.2 기획 수정으로 추가된 부분으로 생성된 중복 코드를 최소화 할 것 (클래스의 확장,유연성이 부족)

6.3 작은 부분도 함수화 할 것

6.4 언리얼에서 제공 하는 클래스들을 목적에 맞게 명확하게 사용하지 할 것

6.5 언리얼 카메라 뷰타겟 및 카메라 매니저 관련 내부 동작에 대한 공부 필요

6.6 블루프린트 최소화 (레벨 시퀀스 동작에 대한 클래스를 따로 만들지 않고 블루프린트로 작성한 부분 등)

6.7 코드 작성 전 의사코드 작성을 꼼꼼히 할 것 (버그 수정에서 불필요한 중복 함수 호출)

6.8 최적화 신경쓰기 (기획 수정으로 특정 상황에 맞게 호출할 수 있는 함수를 Tick에서 중복 호출한 점)

6.9 상속을 여러번 하더라도 각 클래스에서 해야할 최소한의 기능만을 구현할 것 (스킬관련  중복 코드)

6.10 생성자와 비긴플레이에서 다른 클래스의 의존성 최소화
